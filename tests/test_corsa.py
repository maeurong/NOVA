...
def test_una_frequenza_negativa_oltre_la_seconda_non_passa(tmp_path):
    ...
    non si tocca) confronta solo le prime due frequenze per il rapporto e non guarda oltre:
    una negativa in terza posizione non è mai letta, e oggi il verdetto esce `passato`
    (misurato: `solve.controlla_autovalori([5.79829, 8.70049, -10.9984])` rende
    `{'passato': True, ...}`). Il cancello sulle frequenze fisiche va aggiunto a monte, in
    `nova/`, non in `meshrec/`."""
    m, d = _modello_e_deck("trave_appoggiata.nova.json", tmp_path)
    modi = [{"f": 5.79829, "cumulata": {"x": 1.0, "y": 0.0, "z": 0.0}},
            {"f": 8.70049, "cumulata": {"x": 1.0, "y": 0.0, "z": 0.0}},
            {"f": -10.9984, "cumulata": {"x": 1.0, "y": 0.0, "z": 0.0}}]
