...
| [`15-desktop-macos.md`](15-desktop-macos.md) | ticket #38, 08/09/2026: **fatta**. Copre «cosa comporta per NOVA diventare una .app macOS firmata, notarizzata e installabile, quando i solutori restano fuori e non firmati». Misure: PATH e cwd di un'app lanciata dal Finder che spezzano `shutil.which` e `Path.cwd()`; firma ad-hoc `linker-signed` dei solutori installati; binario arm64 senza firma e binario in quarantena uccisi con SIGKILL, in silenzio. Doc: hardened runtime ed entitlements per un Python impacchettato, `notarytool` e stapling, Gatekeeper dopo Sequoia, py2app/PyInstaller/briefcase, aggiornamenti, 80-100 MB, 99 USD l'anno. Fonti catturate in [`fonti/`](fonti/) con sidecar |
