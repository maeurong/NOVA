...
Sette task su sette con `subagent-driven-development` (ledger in `.superpowers/sdd/2026-09-09-t5-giornata-11d-importatore/progress.md`): 12 commit da `a8905d1`, **610 test JS** (da 570) e **698 pytest** (invariati: nessun file sotto `nova/`). Il Task 7 l'ha fatto il controller su **Chrome vero** (estensione accesa) con il server del worktree sulla 8818: il vuoto, il parziale, il sintetico (conferma singola, «conferma tutte», `⌘Z` × 2, `V` su un confermato, `/api/check` con `vincoli` passato), salva e riapri, i due errori, `⌘I` da dentro il campo, il rilievo vero di `lab_telaio_v2` (uguale alla fixture del vuoto); 1920 px e zoom 200 % (viewport 640×400, dpr 2) sul Chrome headless via CDP: niente scorrimento orizzontale né testo tagliato, il rendiconto scorre nel pannello.

**Quota.** `claude-opus-5` ha toccato il limite settimanale la sera del 09/09: il fix del Task 4 e il Task 5 sono caduti a metà; il fix l'ha finito il controller, il Task 5 un secondo implementer Sonnet ripartendo dai test scritti dal primo (due bug ereditati corretti: l'import di `daRisposta`, le due callback in `AZIONI`). Task 6, tutte le review e le re-review su Sonnet.

**Ruling** (tutti nel ledger): R1 niente `CASI_EDITOR` per il rendiconto, test WCAG dedicato; R2 «1,19» (due decimali); R3 il campo del percorso si **svuota** dopo l'importazione (la decisione 4 diceva «resta nel campo»: un `.nova.json` sopra il prior sarebbe una perdita); `daRisposta` non guarda `esito` (gli errori di `importa` escono 400 dal server e `chiediJson` solleva); «conferma tutte» = un `esegui` per proposta (N voci nella Storia, `⌘Z` le disfa una per una); l'etichetta del nodo evita il simbolo del vincolo (`versoLibero(m, n, occupate)` con il basso occupato per i nodi con simbolo, pieno o proposto); la selezione del rilievo non è una «selezione» per la barra (`contestoBarra` → «sempre»); valore e soglia delle scartate a due decimali senza zeri in coda, i millimetri delle giunzioni a `conciso`.

**Difetti visti solo a schermo** (Task 7, chiusi in `953e539`): `scrollIntoView` della Storia trascinava `#pannello` in fondo e dopo l'importazione il rendiconto spariva; `#file` con `flex: 0 1 auto` si faceva mangiare dall'albero lungo (42 e 80 nodi) e «importa» spariva proprio a modello arrivato; la barra prometteva B/A/V/M/R/⌫ col rilievo selezionato. Nessuno dei tre era visibile con la fixture 2×1 delle giornate precedenti: li ha portati fuori il modello grande.

**Premesse false trovate dalle review**: «8 scartate, non 14» (sono 8 regioni e 14 righe per controllo: il brief del Task 6 le confondeva); il dispatch dei percorsi delle fixture come file (`prior_*` sono cartelle con `12_wall.json` dentro); il conteggio dei tratti (`>= 3` per un incastro che ne ha 6); `proposte: null` non prende il default del parametro.

**Fix che hanno rischiato un difetto nuovo**: il formattatore a due decimali delle scartate applicato anche ai millimetri delle giunzioni («proiezione 154 mm» → «153,6»): fermato dal test `testoMancano e testoGiunzione`, da qui `num` e `misura` separati in `rilievo.js`.

**Debiti dichiarati, non pagati qui**: i simboli dei vincoli in px fissi si sovrappongono quando i nodi stanno a 80 mm (il sintetico: 21 nodi al piede), come le etichette dei nodi — un simbolo che si adatta alla densità va con la passata sull'uso; il rendiconto vale per la sessione (dichiarato nell'ispettore); nessun test automatico su `app.js` (terza giornata: la cucitura si prova a mano); `riassunto` non riusa `testoMancano`; `editorVincolo` a 58 righe; con una sezione o un materiale selezionati la barra promette ancora B/A/V/M (11a/11b, non del ramo); due `innerHTML` in `pannello.js` della 11c (fuori diff, da guardare); il corpo del commit `b1837c4` senza diacritici (storia non riscritta); a zoom 200 % la barra dei comandi occupa quattro righe su cinque (11c). «Fuori da questa seduta» del Task 7 resta valido: Check Model e corsa in interfaccia (12), la giunzione come entità, cerniera e carrello con un simbolo proprio, il prior dal corpo della richiesta.

**Verdetto di ramo**: security approvato; test-writer approvato (10/10 mutanti coperti); spec conforme (0 mancante, 0 oltre lo scope); code approvato (1 medio = `app.js` senza test, 5 bassi); craft non approvato (1 alto, 2 medi, 2 bassi) → fix di fine ramo `3446a5a` (l'alto, un medio, un basso, tre bassi del code; un medio parcheggiato con ruling, un basso è storia).
