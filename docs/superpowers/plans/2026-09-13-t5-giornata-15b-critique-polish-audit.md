...
**Fatto**, in otto task più un polish e tre giri nati dalle prove con gli occhi. In aula il **disegno è tornato più grande del testo che lo sovrasta**: la fascia delle strisce scende da 367 a **215 px** e il telaio sale da 126 a **276** su un riquadro di 403. La leva non è stata quella che il piano prevedeva — spostare la legenda dei colori rendeva **+0 px** proprio nel caso di collaudo — ma la **legenda degli stati**, che da sola valeva 152 px su 346 e ora sta in una riga. La coda chiara di viridis sta su una **piastra opaca di `--fondo`**: la piastra d'inchiostro che il piano proponeva **ribaltava** il difetto (giallo da 1,11 a 14,59, ma viola da 10,91 a 1,21 e bordo della rampa da 13,19 a 1,00). Lo **srotolato** e la **curva della pushover** tornano visibili in aula con misure proprie (160 e 240 px), e in aula il numero del taglio massimo **esce dal grafico** e va sotto l'asse, al centro — collocazione scelta misurando **tre candidati su 14 passi**: dentro il grafico 8 sovrapposizioni, sopra l'asse 3, sotto l'asse **0**. Lo **spessore dei cilindri** è finalmente provabile, il **costo per fotogramma** è registrato e non ottimizzato, le variabili CSS si leggono **una volta per cambio di layout**, la forma di un **modo** è dritta fra i nodi, e le ragioni del **Confronto** hanno una sola grafia.

**Misurato** (Chrome headless, corse vere con OpenSees, doppia lettura su ogni numero): in aula i nomi dei nodi stanno a **45,98-46,00 px** con **qualunque ordine di gesti**; la piastra sta **42 px sotto** il nome più vicino; i cilindri rendono **6,04-6,13 px** contro 6 voluti, anche dopo mezzo giro di camera; il costo per fotogramma è **16,55-17,05 ms** di mediana su cinque corse, cioè il periodo del vsync. Conteggi finali: **926 test JS** (base 889), **757 pytest + 3 saltati** (base 748), **26 copioni di fumo** (base 23).

**Trovato dalle prove che i test non fanno.** La critique e l'audit hanno prodotto 14 rilievi misurati, 5 `Important`. La review di ramo a cinque ha trovato un difetto di **correttezza in produzione protetto da un test verde**: il corpo del carattere si leggeva dall'unica striscia che in aula **non** viene ingrandita, quindi la correzione era inerte e gli ostacoli restavano gonfiati di 1,44× — con un mock che iniettava nel test un valore che il browser non produce mai. La review dei test ha provato **37 mutanti** e ne ha trovato uno che sopravvive a tutte e tre le reti: il segnale che tiene dritta la forma dei modi era sorvegliato **dove si scrive**, non dove si usa. E la **prova a mano** ha trovato due regressioni introdotte da questa stessa giornata, che nessuna delle tre reti vedeva: entrando in aula **prima** di correre il disegno restava alla scala di prima (fino a 29,75 px, −35 %), e in quello stato la piastra opaca **cancellava** il nome di un nodo. Il motivo per cui erano invisibili è di metodo: **i copioni provano l'ordine di gesti comodo** — corri, poi entra in aula — mentre chi presenta fa il contrario.

**Ruling principali**: la legenda dei colori si **accorcia**, non si copre (un coperchio opaco fa sparire i nomi, e un nome che sparisce è peggio di un nome affollato); il riuso delle mesh **esce dal ramo** perché il costo è già al vsync, con la soglia che lo riaprirebbe scritta in issue; il segnale della forma modale è **esplicito** e non dedotto dalle rotazioni nulle — che valgono zero anche in una trave incastro-incastro vera; la legenda in basso è un **ostacolo** come le strisce in alto, e sotto la soglia sparisce **lei**, non il nome del nodo; ogni review che esegue codice lo fa su un **archivio isolato** del commit, non sul worktree condiviso.

**Debiti dichiarati, tutti con la misura dentro**: **#87** (riuso delle mesh, con la banda 16,55-17,05 e le soglie che la riaprono), **#88** (gli artefatti del caso studio si dichiarano generati ma niente nel repo li rigenera), **#89** (la griglia che a 640×400 riduce il piano a 40×10 px, i titoli strutturali a 11 px, i bersagli sotto 24 px, il fuoco perso dalla palette, i 33 verdetti senza riepilogo), **#90** (sotto i 1920 px l'aula non scrive due etichette che la scrivania mostra: in aula «q −0,5305 N/mm» chiede il 51 % della larghezza del piano). Restano scritti anche: il ramo «nessun cilindro in scena» della sonda senza prova, `TELAIO_MINIMO` tarato su un caso solo, i millimetri per pixel ancora calcolati su valori arrotondati (chiuso il sintomo, non la radice), il rosso della deformata stantia non esercitato in bianco e nero, e nel 3D la deformata che in scala di grigi si distingue poco dall'ombra.

**Lezioni.** Questa giornata ha trovato **otto modi diversi** in cui una prova non prova niente, e vale la pena elencarli: un'asserzione che confronta `0 === 0` in una fixtura dove il caso non si presenta; un test che copre l'unità nuova ma non il patto; un test che regge su una costante che il codice stesso ha reso falsa; un controllo che guarda solo l'ultimo passo di una pushover, dove il difetto non c'è; un'asserzione che **trascrive la formula** del codice; un mutante mal costruito che lascia in piedi la chiamata buona; un mutante **impossibile**, che muta una riga che non governa il comportamento da provare; e un **mock che descrive un mondo che il browser non produce**, il più costoso, perché protegge il difetto rendendolo invisibile. Il filo comune: **il punto di misura conta più del test**. Due volte la misura giusta stava altrove — ai quarti dell'asta e non a metà, ai passi bassi della pushover e non all'ultimo — e due volte una costante iniettata ha tenuto in piedi una prova morta. Fuori dai test: citare `file:riga` nei commenti **invecchia più in fretta di quanto si scriva** (undici rimandi di questo ramo erano già morti a fine giornata, incluso quello del commit che correggeva i rimandi morti), e la cura è citare il selettore; e un worktree condiviso fra agenti paralleli **condivide l'indice di git**, quindi «commit per percorso» non basta a garantire che un commit contenga solo il suo lavoro.
