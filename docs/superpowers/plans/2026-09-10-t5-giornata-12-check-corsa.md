...
**Il `kbd` con i glifi** è diventato codice: `nomeTasto` (`tastiera.js`) dà a ogni tasto un nome accessibile in parole — «comando invio», «maiuscolo comando invio» — sui `kbd` della barra, della palette e dello stato vuoto (la prova con VoiceOver resta da fare a orecchio). **Resta, perché non è un commit**: `app.js` senza test automatici — la strada è un test di fumo in Chrome headless via CDP lanciato da pytest (salta se Chrome manca), primo punto della giornata 13. Il tetto dei giri del polling resta fuori per scelta (il soffitto è nel server). Nota per l'autore: `python -m nova` apre una scheda del browser a ogni avvio; le prove di questa giornata ne hanno aperte alcune sul tuo Chrome (8818, 8819, 8820) che si possono chiudere.
