...
## Esito (09/09/2026, chiusura del ramo)

Nove task su nove eseguiti con `subagent-driven-development` (ledger in `.superpowers/sdd/2026-09-09-t5-giornata-11c-azioni-palette/progress.md`): 23 commit da `c1a0337`, **569 test JS** (da 444) e **698 pytest** (invariati: nessun file sotto `nova/`). Il Task 9 è stato fatto dal controller con un Chrome headless pilotato via CDP (l'estensione Claude in Chrome era scollegata per tutta la seduta): fixture 2×1, portale da zero con `Z`/`Q`/`K`, editor, palette, `⌫`/`⌘Z`, salvataggio con la forma della fixture, Check Model `riferimenti`/`carico_termico` passati, 1280/1920/zoom 200 % (viewport 640×400, dpr 2) senza scorrimento orizzontale né testo tagliato.

**Ruling che hanno cambiato il patto** (tutti nel ledger): R1 il nome accessibile del coefficiente comincia dal visibile («‹azione›: coefficiente dell'azione Z‹id› nella combinazione ‹nome›», unico anche con azioni omonime); R2 la palette **abbandona** campo e modo (la decisione 4 diceva «elenca tutto»: elenca `VOCI_PALETTE`, cioè `TASTI` senza `direzione`, `conferma`, `palette`, e valuta il contesto a gesto chiuso); a query vuota la palette mostra tutte le voci (nessun tetto); `impostaTermine` mostra la somma di due termini sulla stessa azione, rifiuta di scriverci sopra e lascia svuotare (toglie entrambi); le guardie di `eliminaAzione`/`eliminaCombinazione` guardano anche la modale (`masse_da_azioni`) e la pushover (`caso_gravita`) — buco del piano, non dell'esecuzione; `⌘Z` si chiama «disfa» (l'etichetta «annulla» era doppia); il termico entra con l'avviso «la verifica del modello rifiuta il carico termico in questa versione»; le migliaia stanno in ogni campo numerico (`leggiNumero` rilegge U+202F) e il meno tipografico rientra dalla propria porta (`numeri.js` normalizza U+2212); `cifre` e `conciso` vivono in `numeri.js`; la voce selezionata dell'albero ha il filetto rosso e il grassetto, non il rosso come testo; il titolo dei carichi è un `<p>` sopra il piano, non un `<text>` nel `viewBox`; «aggiungi carico» è un select più un bottone (su Windows/Linux le frecce di un select chiuso sparano `change`).

**Premesse false trovate dalle review** (a memoria per la prossima giornata): `leggiEspressione("2e6")` è `null`; `assert.throws(fn, /re/)` legge il messaggio, non il rimedio; il DOM finto di `pannello.test.js` prova anche il riquadro della palette (la deroga del piano era falsa); `Mx 2e6` nel piano; 14 righe di puntamento sbagliate (annotazione dell'architect); il brief riga 28 del Task 8 (la Storia con la categoria); l'emulazione dello zoom con `zoom` CSS non riduce i `vh` (falso difetto sulla palette); il `?v=` non scavalca la cache dei moduli (serve `Network.setCacheDisabled`).

**Fix che hanno creato un difetto nuovo** (memoria «una fix è codice nuovo»): il round 1 del Task 2 (`?? []`) ha spostato la sorgente del filtro dei termini su `vecchia.termini` → oggetti condivisi fra snapshot, preso al round 2 con un test d'identità; il round 1 del Task 7 ha lasciato un `*/` di troppo che inghiottiva la regola di `#palette-stato`, preso al round 2 con un test che conta le aperture dei commenti.

**Debiti dichiarati, non pagati qui**: `azioneCorrente` fuori dalla cronologia (`⌘Z` non la riporta: il fallback all'ultima azione la copre per caso); nome accessibile posizionale dei carichi («Fx del carico 2»: dopo un `⌘Z` che toglie il primo, il bersaglio della scrittura può cambiare carico — un id per carico è T6); nessun test automatico su `app.js` (il test-writer ha fatto estrarre `caricoVuoto` e `azioneInVista` come funzioni pure); quattro DOM finti nei test (il test-writer sconsiglia di unificarli); `cambiata` con `JSON.stringify` cieco all'aliasing (difeso dai test d'identità sui tre riduttori che toccano liste annidate); `rif` un `<option>` per nodo per carico (commento ponytail con il soffitto); `analisiCheUsano` con due rami noti e nessun rifiuto su un quarto tipo di analisi (oggi non esiste); i soggetti di 15 commit oltre i 72 caratteri e due corpi senza diacritici (storia non riscritta); `S` «300 × 500» ripetuto crea sezioni uguali invece di riusare (11b); la Storia dice «asta da 1» per un'estrusione (11a). Nella passata sull'uso dopo T5 (mappa #31).

**Verdetto di ramo** (re-review del fix di fine ramo): 20/20 punti chiusi, nessun difetto nuovo, **mergiabile**; i tre residui (nomi al posto degli id in due messaggi di `eliminaAzione` e in `eliminaCombinazione`, una ricerca doppia in `impostaTermine`, il selettore `.editor-campi > button` stretto al box del carico) chiusi dal controller nel commit successivo.

**Verifica su Chrome vero (09/09, estensione riaccesa)**: due difetti che l'headless non aveva mostrato, chiusi nel commit successivo — il campo di comando nascosto teneva il fuoco (`Z` dopo un Invio non apriva niente: `chiudiComando` ora fa `blur()`); due distribuiti sulla stessa asta si scrivevano addosso (frecce del secondo più lunghe, etichette impilate sopra la coda più alta). Nota per la passata sull'uso: dopo «apri» il fuoco resta nel campo del percorso (11a), e i tasti dei comandi ci finiscono dentro finché non si clicca nel piano.
