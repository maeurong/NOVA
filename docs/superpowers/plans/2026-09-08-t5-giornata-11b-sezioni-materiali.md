...
## Esito (08/09/2026, chiusura del ramo)

Eseguito con `superpowers:subagent-driven-development`: nove task, nove review per task, dieci fix round, cinque review di ramo (sicurezza, correttezza, copertura, artigianato, conformità) e un fix di fine ramo. Ledger: `.superpowers/sdd/2026-09-08-t5-giornata-11b-sezioni-materiali/progress.md` nel worktree `interfaccia`.

**Premesse false di questo piano, corrette in corsa (chi legge il piano le sappia):**
- «`f_ctm` è `None` solo sull'acciaio: è la chiave con cui il catalogo si divide … un campo `tipo` che il catalogo non ha» — falso: `VoceMateriale.famiglia` esiste (`meshrec/core/materiali.py:134`). Il catalogo si divide per `famiglia`.
- Task 1, «corpo con un campo in più → 400» — è **422** dal gestore di `RequestValidationError` (`nova/server.py:282-285`). Corretto prima del dispatch.
- Task 4, `puntiConcrete02` chiudeva a `[-0, -0]`: `zero()` aggiunta prima del dispatch.
- Task 5, cinque regex dei test cercavano nel **rimedio** e non nel messaggio (`assert.throws` guarda `err.toString()`): aiutante `rifiuta` nel test.
- Task 8, `testoNumero` con `toExponential`/`toPrecision` non rientra da `leggiNumero`: si stampa con `stampaNumero` a quattro cifre significative.
- Task 9, il frammento di `confermaSezione` col nome di una sezione esistente e nessuna asta scriveva una voce di Storia vuota e selezionava la sezione sbagliata.
- Task 10 Step 3, «`h` = 100 → messaggio copriferri» vale per i numeri del piano (copriferro 40, Ø16), non per il MURO 1 (copriferro 20, Ø4, Ø12: 60 < 100 è lecito). Il programma ha ragione.
- Global Constraint «Backend: solo Task 1 tocca `nova/`»: derogato **due volte** con ruling nel ledger — `legami.py`/`server.py` nel Task 1 come previsto, e nel fix di fine ramo `nova/modello.py` per il validator `tipo` ↔ `famiglia` (500 nudo e numeri finti su `acciaio` + `C25/30`) e le frasi `int_type`/`float_type` (il limite dichiarato parlava inglese).

**Ruling che hanno cambiato il patto** (tutti nel ledger): «1 asta» al singolare; `#pannello-editor` con una riga di `app.js` nel Task 8; `⌫` su sezione e materiale nel Task 9 (il debito 5 di «Fuori da questa seduta» è quindi chiuso, non aperto); `.avviso` come testo inchiostro con filetto rosso, perché il rosso a 11 px misura 4,28:1; guardia «lato vuoto che resta vuoto» in `suFila`; `chiediJson` unica in `file.js`; `millimetri` in `numeri.js`; un comando che non cambia niente non entra nella Storia (`applica` restituisce `c` se il riduttore restituisce il modello per riferimento).

**Debiti registrati per la passata sull'uso dopo T5** (mappa #31, «Not yet specified»): la veste sta nell'ispettore del materiale e non nel blocco «Unità»; gli stati vuoti dell'albero non nominano `S`/`C`; `contestoBarra` ha un solo contesto «selezione» (la barra promette `B`/`A`/`V`/`M` con una sezione selezionata); l'albero selezionato in rosso a 13 px misura 4,28:1; `confermaSezione`/`confermaMateriale`/`confermaDanno` sono logica pura dentro `app.js` senza test `node` (estraibile in `modo.js`); `pannello.js` ha sei modi di stampare un numero senza una regola; tre tabelle parallele sui quattro tipi selezionabili; la `Map` dei legami non si pota mai; `legame: null` in `modificaMateriale` è un no-op; con due lati doppi il messaggio nomina il primo in ordine `LATI`, il deck il primo dichiarato; la geometria è giudicata anche su campi non toccati; un campo ignoto nei riduttori è ignorato in silenzio; `editorMateriale` 73 righe; zoom 200 % e VoiceOver non misurati (surrogato a 160 px e `role="presentation"` letto, non ascoltato). Dal fix di fine ramo: il fuoco dopo un bottone che sparisce («togli staffe», «togli danno», «torna ai valori di tabella») va al primo controllo dell'editor, che è un `<select>`, e una lettera lì cambia la classe (comportamento nativo del `<select>`); `catalogo.valori` chiama `materiali.trova` senza guardare `personalizzato`, quindi una classe fuori catalogo non arriva a `/api/materiale/legame` anche se il modello la accetta (la story 20 vuole valori sovrascritti su una classe di norma: non è un buco della story, è un limite da scrivere in T9).

**Fix di fine ramo, tre round** (`dec93a2`, `98952a9`, `0210fe5`, `2630403`; `35f6965`, `d81130d`; `55ea230`): il round 2 ha chiuso il caso `personalizzato` del validator (la famiglia si confronta ogni volta che la voce esiste) e il round 3 la seconda causa del fuoco che scappava (il gruppo «valori» spariva per un ridisegno mentre il legame era in volo: ora si costruisce dalle chiavi di tabella dell'ultima risposta per la stessa classe). Debito residuo: quella tabella tiene gli override per un giro di POST dopo «torna ai valori di tabella»; «scritto a mano» è visibile ma fuori dal nome accessibile. Conteggi finali: **444 `node --test`**, **698 pytest** (695 pass + 3 skip).
