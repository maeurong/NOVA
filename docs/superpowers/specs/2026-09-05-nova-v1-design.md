...
Fixture versionate: modelli `.nova.json` (telaio 2×1 sano, asta a lunghezza zero, nodo libero, nodi coincidenti con asta duplicata, trave appoggiata), prior sintetico generato una volta da MeshRec (task #16, con comando, commit e impronta annotati), prior parziale a mano, CSV Abaqus d'esempio. Il prior vero e il deck solido stanno in `lab_telaio_v2/` non versionata: i test che li usano sono saltati se la cartella manca. I test sul binario riprendono la forma di `tests/feasibility/` di MeshRec.
